...
data class RequestAllPermissionsResult(val getPermissionState: (String) -> PermissionState?): JSResult {
    private val permissions: JSObject = JSObject()

    init {
        val writeCalendarState = getPermissionState(CalendarPermissionScope.WRITE_CALENDAR.value) ?: throw PluginError.UnhandledPermissionState
        permissions.put(CalendarPermissionScope.WRITE_CALENDAR.value, writeCalendarState.toString())
        val readCalendarState = getPermissionState(CalendarPermissionScope.WRITE_CALENDAR.value) ?: throw PluginError.UnhandledPermissionState
