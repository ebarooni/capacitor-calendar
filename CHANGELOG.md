...
- Clearable optional fields on `modifyReminder(...)`: pass `null` to clear `notes`, `url`, `location`, `alerts`, `recurrence`, `startDate`, `dueDate`, or `completionDate` (`alerts` also clears with `[]`)
